...
import PagosPendientesList from "../pagos-pendientes/PagosPendientesList";

export default async function PagosPage() {
  // 🔐 ZERO TRUST: Validar rol y obtener empresa_id
  ...
  const { data: pagos, error } = await supabase
    ...
    `)
    .eq("prestamos.empresa_id", empresa_id) // 🔐 Filtro por empresa
      ...
      <div className="px-4 pb-32">
        <div className="mb-6 flex items-center gap-2 px-4 py-3 bg-ios-blue/5 rounded-2xl border border-ios-blue/10">
          ...
          <p className="text-[12px] font-bold text-ios-blue uppercase tracking-wider">
            Solo visualizas pagos marcados como "Pendiente de Validación" de su empresa.
          </p>
        </div>
